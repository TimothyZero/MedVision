...
def test2d():
    result, img_filename, seg_filename = load_2d_image_with_seg(to_cuda=True)

    transform = CudaForwardCompose([
        CudaResize(factor=(2.0, 2.0)),
        CudaRandomNoise(p=1.0),
        CudaRandomFlip(p=1.0),
        CudaRandomRotate(p=1.0, rotate=40)
    ])
    tic = time.time()
...
def test3d():
    result, img_filename, seg_filename = load_3d_image_with_seg(to_cuda=True)
...
if __name__ == "__main__":
    ...
    save_to = os.path.join(__dir__, 'CudaPipeline')
