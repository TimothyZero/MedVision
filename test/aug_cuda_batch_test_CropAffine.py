...
def test2d():
    result, img_filename, seg_filename = load_2d_image_with_seg(batch=2)

    order = 3
    transform = BatchCudaCropRandomWithAffine(patch_size=(256, 256),
                                              shift=0.1, scale=0.1, rotate=40,
                                              order=order,
                                              times=3)
    tic = time.time()
...
def test3d():
    result, img_filename, seg_filename = load_3d_image_with_seg(batch=2)

    order = 3
    transform = BatchCudaCropRandomWithAffine(patch_size=(96, 128, 128),
                                              shift=0.1, scale=0.1, rotate=40,
                                              order=order,
                                              times=3)
    tic = time.time()
