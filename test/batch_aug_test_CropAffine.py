...
from medvision.transforms_cuda_batch import BatchCudaCropRandomWithAffine, Display
...
def test2d():
    ...
    result = {
        ...
        'gt_seg':     torch.cat([seg, seg, seg], dim=0),
        'seg_fields': ['gt_seg'],
    }
    order = 1
    transform = BatchCudaCropRandomWithAffine(patch_size=(256, 256), shift=0.1, scale=0.1, rotate=40)
    tic = time.time()
...
def test3d():
    ...
    volume2tiled(img[0, 0].cpu().numpy(), f'{save_to}/{img_filename}.raw.png', 10)
    ...
    order = 3
    transform = BatchCudaCropRandomWithAffine(patch_size=(96, 128, 128), shift=0.1, scale=0.1, rotate=40)
    tic = time.time()
    transformed_result = transform(result)
    toc = time.time()
    print(transform)
...
if __name__ == "__main__":
    ...
    save_to = os.path.join(__dir__, 'BatchCudaCropRandomWithAffine')
