...
def test_2d():
    x_2d = torch.rand((1, 3, 64, 64)).cuda()

    dcn = DeformConv2dPack(in_channels=3, out_channels=16, kernel_size=3, stride=1, padding=1).cuda()
    print(dcn(x_2d).shape)
    with torch.cuda.amp.autocast():
        print(dcn(x_2d).shape)
    dcn = ModulatedDeformConv2dPack(in_channels=3, out_channels=16, kernel_size=3, stride=1, padding=1).cuda()
    print(dcn(x_2d).shape)
    with torch.cuda.amp.autocast():
        print(dcn(x_2d).shape)
...
def test_3d():
    x_3d = torch.rand((1, 3, 64, 64, 64)).cuda()

    dcn = DeformConv3dPack(in_channels=3, out_channels=16, kernel_size=3, stride=1, padding=1).cuda()
