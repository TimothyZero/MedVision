...
from medvision.aug_cuda import Normalize, MultiNormalize, Display
...
def test2d():
    ...
    transform = MultiNormalize(means=[(0.5, ), (0.3, )], stds=[(0.3, ), (0.5, )])
    tic = time.time()
...
def test3d():
    ...
    transform = Normalize(mean=(-400,), std=(300,))
    tic = time.time()
    transformed_result = transform(result)
    toc = time.time()
    print(transform)
...
if __name__ == "__main__":
    ...
    save_to = os.path.join(__dir__, 'Normalize')
