...
def call_roi_align_2d(name, obj, fs, rois, device='cpu'):
...
def test2d(img_path):
    ...
    call_roi_align_2d(filename + 'RoIAlignTorchvision', RoIAlignTorchvision, fs, rois)
    call_roi_align_2d(filename + 'RoIAlignCuda', RoIAlignCuda, fs, rois, device='cuda')
    ...
    rotated_rois2 = torch.tensor([
                                     [0, 290., 240., 256.0, 256.0, 40.0],
                                 ] * 1000)
    call_roi_align_2d(filename + 'RoIAlignRotated.1', RoIAlignRotated, fs, rotated_rois1, device='cuda')
    call_roi_align_2d(filename + 'RoIAlignRotated.2', RoIAlignRotated, fs, rotated_rois2, device='cuda')

    # image coord is y, x order
    io.imsave(f'{save_to}/{filename}.crop.png', (255 * image[112:368, 162:418]).astype(np.uint8))


def call_roi_align_3d(name, obj, fs, rois, device='cuda'):
    fs, rois = fs.float().to(device), rois.to(device)

    tic = time.time()
    r = obj((30, 178, 178), 1.0, 1)
...
def test3d(img_path):
    ...
    rotated_rois2 = torch.tensor([[0, 99., 99., 75., 178, 178, 30, 0, 0, -20/180]])
    rotated_rois3 = torch.tensor([[0, 99., 99., 75., 178, 178, 30, 20/180, 0, -20/180]])
    call_roi_align_3d(filename + 'RoIAlignRotated3d.1', RoIAlignRotated, fs, rotated_rois1)
    call_roi_align_3d(filename + 'RoIAlignRotated3d.2', RoIAlignRotated, fs, rotated_rois2)
    call_roi_align_3d(filename + 'RoIAlignRotated3d.3', RoIAlignRotated, fs, rotated_rois3)
...
if __name__ == "__main__":
    ...
    test2d("../samples/21_manual1.png")

    test3d("../samples/luna16_iso_crop_img.nii.gz")
    test3d("../samples/luna16_iso_crop_lung.nii.gz")
