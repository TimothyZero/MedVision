...
#include "deform_conv_3d_cuda_kernel.cuh"
...
template <typename scalar_t>
void deformable_3d_col2im_cuda(cudaStream_t stream,
  ...
  <<<GET_BLOCKS(num_kernels), CUDA_NUM_THREADS, 0, stream>>>(
    ...
    depth_col, height_col, width_col, grad_im);
  cudaError_t err = cudaGetLastError();
  if (err != cudaSuccess)
  {
...
template <typename scalar_t>
void deformable_3d_col2im_coord_cuda(cudaStream_t stream,
...
std::vector<at::Tensor> deform_conv_3d_cuda_backward(const at::Tensor &input,
                                             const at::Tensor &weight,
                                             const at::Tensor &bias,
                                             const at::Tensor &offset,
