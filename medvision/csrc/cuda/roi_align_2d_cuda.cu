/*
ROIAlign2D implementation in CUDA from pytorch framework
...
template <typename T>
__global__ void RoIAlign2DForward(
    const int nthreads,
    const T* input,
...
template <typename T>
__global__ void RoIAlign2DBackward(
    const int nthreads,
    const T* grad_output,
    const T spatial_scale,
    const int channels,
...
at::Tensor ROIAlign2D_forward_cuda(const at::Tensor& input, const at::Tensor& rois, const float spatial_scale,
  ...
  at::CheckedFrom c = "ROIAlign2D_forward_cuda";
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(input.type(), "ROIAlign2D_forward", [&] {
    RoIAlign2DForward<scalar_t><<<grid, block, 0, stream>>>(
        output_size,
...
at::Tensor ROIAlign2D_backward_cuda(
  ...
  at::CheckedFrom c = "ROIAlign2D_backward_cuda";
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(grad.type(), "ROIAlign2D_backward", [&] {
    RoIAlign2DBackward<scalar_t><<<grid, block, 0, stream>>>(
        grad.numel(),
