...
template <typename T>
__device__ inline float devIoU_2d(T const* const a, T const* const b) {
...
__global__ void nms_kernel_2d(
  ...
  if (threadIdx.x < row_size) {
    ...
    for (i = start; i < col_size; i++) {
      if (devIoU_2d<T>(cur_box, block_boxes + i * 5) > nms_overlap_thresh) {
        t |= 1ULL << i;
      }
    }
...
at::Tensor nms_2d_cuda(const at::Tensor boxes, float nms_overlap_thresh) {
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      boxes_sorted.type(), "nms_kernel_2d_cuda", [&] {
        nms_kernel_2d<scalar_t><<<blocks, threads, 0, stream>>>(
            boxes_num,
            nms_overlap_thresh,
