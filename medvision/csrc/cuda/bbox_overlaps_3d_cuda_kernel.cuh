...
template <typename T>
__global__ void bbox_overlaps_3d_cuda_kernel(const T* bbox1, const T* bbox2,
  ...
  if (aligned) {
    CUDA_1D_KERNEL_LOOP(index, num_bbox1) {
      ...
      T b1_z1 = bbox1[base1 + 2];
      T b1_x2 = bbox1[base1 + 3];
      T b1_y2 = bbox1[base1 + 4];
      T b1_z2 = bbox1[base1 + 5];
      T b1_area = (b1_x2 - b1_x1 + offset) * (b1_y2 - b1_y1 + offset) * (b1_z2 - b1_z1 + offset);

      int base2 = b2 * 6;
      T b2_x1 = bbox2[base2];
      T b2_y1 = bbox2[base2 + 1];
      T b2_z1 = bbox2[base2 + 2];
      T b2_x2 = bbox2[base2 + 3];
      T b2_y2 = bbox2[base2 + 4];
      T b2_z2 = bbox2[base2 + 5];
      T b2_area = (b2_x2 - b2_x1 + offset) * (b2_y2 - b2_y1 + offset)  * (b2_z2 - b2_z1 + offset);

      T left = fmaxf(b1_x1, b2_x1), right = fminf(b1_x2, b2_x2);
      T top = fmaxf(b1_y1, b2_y1), bottom = fminf(b1_y2, b2_y2);
      T front = fmaxf(b1_z1, b2_z1), back = fminf(b1_z2, b2_z2);
      T width = fmaxf(right - left + offset, 0.f);
      T height = fmaxf(bottom - top + offset, 0.f);
      T depth = fmaxf(back - front + offset, 0.f);
