...
#include "modulated_deform_conv_3d_cuda_kernel.cuh"
...
void modulated_deformable_3d_im2col_cuda(
...
void modulated_deformable_3d_col2im_cuda(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      data_col.scalar_type(), "modulated_deformable_col2im_gpu", ([&] {
        ...
        <<<GET_BLOCKS(num_kernels), CUDA_NUM_THREADS, 0, at::cuda::getCurrentCUDAStream()>>>(
            ...
            depth_col, height_col, width_col, grad_im_);
      }));
...
void modulated_deformable_3d_col2im_coord_cuda(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      data_col.scalar_type(), "modulated_deformable_col2im_coord_gpu", ([&] {
        ...
        <<<GET_BLOCKS(num_kernels), CUDA_NUM_THREADS, 0, at::cuda::getCurrentCUDAStream()>>>(
            ...
            batch_size, 3 * kernel_d * kernel_h * kernel_w * deformable_group, deformable_group,
...
void modulated_deform_conv_3d_cuda_forward(
  ...
  if (kernel_h_ != kernel_h || kernel_w_ != kernel_w || kernel_d_ != kernel_d)
    AT_ERROR("Input shape and kernel shape wont match: (%d x %d x %d vs %d x %d x %d).", kernel_h, kernel_w, kernel_d, kernel_h_, kernel_w_, kernel_d_);
  ...
  if (ones.ndimension() != 3 ||
      ones.size(0) * ones.size(1) * ones.size(2) < depth_out * height_out * width_out) {
    // Resize plane and fill with ones...
    ones = at::ones({depth_out, height_out, width_out}, input.options());
  }

  // resize output
  output = output.view({batch, channels_out, depth_out, height_out, width_out}).zero_();
  // resize temporary columns
  columns =
      at::zeros({channels * kernel_d * kernel_h * kernel_w, 1 * depth_out * height_out * width_out},
                input.options());
  ...
  for (int b = 0; b < batch; b++) {
    modulated_deformable_3d_im2col_cuda(
...
void modulated_deform_conv_3d_cuda_backward(
  ...
  grad_input = grad_input.view({batch, channels, depth, height, width});
  columns = at::zeros({channels * kernel_d * kernel_h * kernel_w, depth_out * height_out * width_out},
                      input.options());
  ...
  for (int b = 0; b < batch; b++) {
    ...
    modulated_deformable_3d_col2im_coord_cuda(
    ...
    modulated_deformable_3d_col2im_cuda(
   ...
   grad_output = grad_output.view({grad_output.size(0) * grad_output.size(1),
                                  ...
                                  grad_output.size(4), grad_output.size(5)});
}
