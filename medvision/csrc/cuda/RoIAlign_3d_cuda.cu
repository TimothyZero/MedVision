/*
ROIAlign3D implementation in CUDA from pytorch framework
...
template <typename T>
__global__ void RoIAlign3DForward(const int nthreads, const T* input, const T spatial_scale, const int channels,
...
template <typename T>
__global__ void RoIAlign3DBackward(const int nthreads, const T* grad_output, const T spatial_scale, const int channels,
    const int height, const int width, const int depth, const int pooled_height, const int pooled_width,
...
at::Tensor ROIAlign3D_forward_cuda(const at::Tensor& input, const at::Tensor& rois, const float spatial_scale,
  ...
  at::CheckedFrom c = "ROIAlign3D_forward_cuda";
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(input.type(), "ROIAlign3D forward in 3d", [&] {
...
at::Tensor ROIAlign3D_backward_cuda(
    ...
    const int sampling_ratio)
{
  ...
  at::CheckedFrom c = "ROIAlign3D_backward_cuda";
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(grad.type(), "ROIAlign3D backward 3D", [&] {
    RoIAlign3DBackward<scalar_t><<<grid, block, 0, stream>>>(
        grad.numel(),
