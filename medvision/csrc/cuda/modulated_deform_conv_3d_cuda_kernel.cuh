...
template <typename scalar_t>
__device__ scalar_t get_gradient_weight(scalar_t argmax_d, scalar_t argmax_h, scalar_t argmax_w,
...
template <typename scalar_t>
__device__ scalar_t get_coordinate_weight(scalar_t argmax_d, scalar_t argmax_h, scalar_t argmax_w,
...
// above is the same code in DCN v1 and v2
    ...
    scalar_t *data_col)
{
  ...
  CUDA_1D_KERNEL_LOOP(index, n)
  {
    ...
    const scalar_t *data_mask_ptr = data_mask + (b_col * deformable_group + deformable_group_index) * kernel_d * kernel_h * kernel_w * depth_col * height_col * width_col;
    ...
    for (int i = 0; i < kernel_d; ++i)
    {
      for (int j = 0; j < kernel_h; ++j)
      {
        for (int k = 0; k < kernel_w; ++k)
        {
          ...
          const int data_mask_dhw_ptr = (((i * kernel_h * kernel_w + j * kernel_w + k) * depth_col + d_col) * height_col + h_col) * width_col + w_col;
          const scalar_t offset_d = data_offset_ptr[data_offset_d_ptr];
          const scalar_t offset_h = data_offset_ptr[data_offset_h_ptr];
          const scalar_t offset_w = data_offset_ptr[data_offset_w_ptr];
          const scalar_t mask = data_mask_ptr[data_mask_dhw_ptr];
    ...
    scalar_t *grad_im)
{
  CUDA_1D_KERNEL_LOOP(index, n)
  {
    ...
    const scalar_t *data_mask_ptr = data_mask + (b * deformable_group + deformable_group_index) * kernel_d * kernel_h * kernel_w * depth_col * height_col * width_col;
    ...
    const int data_mask_dhw_ptr = (((i * kernel_h * kernel_w + j * kernel_w + k) * depth_col + d_out) * height_col + h_out) * width_col + w_out;
    const scalar_t offset_d = data_offset_ptr[data_offset_d_ptr];
    const scalar_t offset_h = data_offset_ptr[data_offset_h_ptr];
    const scalar_t offset_w = data_offset_ptr[data_offset_w_ptr];
    const scalar_t mask = data_mask_ptr[data_mask_dhw_ptr];
      ...
      for (int dy = -2; dy <= 2; dy++)
      {
        for (int dx = -2; dx <= 2; dx++)
        {
              ...
              abs(cur_inv_w_data - (cur_w + dx)) < 1)
          {
            ...
            scalar_t weight = get_gradient_weight(cur_inv_d_data, cur_inv_h_data, cur_inv_w_data, cur_d + dz, cur_h + dy, cur_w + dx, depth, height, width);
    ...
    scalar_t *grad_offset, scalar_t *grad_mask)
{
  CUDA_1D_KERNEL_LOOP(index, n)
  {
    ...
    for (int col_c = (offset_c / 3); col_c < channel_per_deformable_group; col_c += col_step)
    {
      ...
      {
        mval += data_col_ptr[col_pos] * deformable_im2col_bilinear(data_im_ptr + cnt * depth * height * width, height, width, depth, height, width, inv_d, inv_h, inv_w);
      }
    ...
    if (offset_c % 2 == 0)
      grad_mask[((((b * deformable_group + deformable_group_index) * kernel_d * kernel_h * kernel_w + offset_c / 2) * depth_col + d) * height_col + h) * width_col + w] = mval;
  }
}