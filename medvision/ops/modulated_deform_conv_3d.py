...
class ModulatedDeformConv3dFunction(Function):
    ...
    def forward(ctx,
        ...
        output = offset.new_empty(
            ModulatedDeformConv3dFunction._infer_shape(ctx, input, weight))
        ctx._bufs = [offset.new_empty(0), offset.new_empty(0)]
        if offset.dtype == torch.float16:
            input = input.half()
            bias = bias.half()
            weight = weight.half()
        _C.modulated_deform_conv_3d_forward(
    ...
    def backward(ctx, grad_output):
        ...
        _C.modulated_deform_conv_3d_backward(
    ...
    def _infer_shape(ctx, input, weight):
        ...
        depth, height, width = input.shape[2:5]
        kernel_d, kernel_h, kernel_w = weight.shape[2:5]
        depth_out = (depth + 2 * ctx.padding -
                      (ctx.dilation * (kernel_d - 1) + 1)) // ctx.stride + 1
        height_out = (height + 2 * ctx.padding -
                      (ctx.dilation * (kernel_h - 1) + 1)) // ctx.stride + 1
        width_out = (width + 2 * ctx.padding -
                     (ctx.dilation * (kernel_w - 1) + 1)) // ctx.stride + 1
        return n, channels_out, depth_out, height_out, width_out


modulated_deform_conv = ModulatedDeformConv3dFunction.apply
...
class ModulatedDeformConv3d(nn.Module):
    ...
    def __repr__(self):
        ...
        s += f'stride={_triple(self.stride)}, '
        s += f'padding={_triple(self.padding)}, '
        s += f'dilation={_triple(self.dilation)}, '
...
class ModulatedDeformConv3dPack(ModulatedDeformConv3d):
    ...
    def __init__(self, *args, **kwargs):
        ...
        out_channels = self.deformable_groups * 4 * self.kernel_size[0] * self.kernel_size[1] * self.kernel_size[2]
